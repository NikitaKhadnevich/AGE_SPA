...
import {
  ACTION_GET_CIVIL_Requested,
  ACTION_GET_CIVIL_REQUESTED_DETAIL,
  ACTION_GET_ROUTE_CIVIL_MENU,
...
const CivInfo = (props) => {
  ...
  const UNQ_UNIT = "unique_unit";
  const UNIQ_TECH = "unique_tech";
  ...
  const handleclick = (e) => {
    ...
    targetPath && targetId === UNQ_UNIT
      ? dispatches(
          ACTION_GET_CIVIL_REQUESTED_DETAIL(dataDetailUnit[targetPath])
        )
      : targetPath && targetId === UNIQ_TECH
      ? dispatches(
          ACTION_GET_CIVIL_REQUESTED_DETAIL(dataDetailTech[targetPath])
        )
      : null;
  };
  //^^^^
          ...
          if (params.id === item.name + item.id) {
            return (
              <>
                <div
                  key={"Wrapper" + civInfo}
                  className={`detailWrapper detailFor${civInfo}`}
                >
                  <ButtonGoBack
                  ...
                  <div key={"Items" + civInfo} className={"items" + civInfo}>
                    <div key={"ItemName" + civInfo} className={`elementName`}>
                      <p key={params.id + civInfo + "name"}>
                        Сооружение {item.name}
                      </p>
                    </div>
                    <div
                      key={"Items" + civInfo + i}
                      className={"item" + civInfo}
                    >

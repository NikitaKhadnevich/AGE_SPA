...
import {
  ACTION_GET_CIVIL_DETAIL_UNIT,
  ACTION_GET_CIVIL_DETAIL_TECH,
} from "../../ducks/civil/actions";
const { civilStupid } = indicator;

export const CivilInfoStupid = (props) => {
  const {
    children: [item, urlCiv, handleclick, civilizations],
  } = props;
  ...
  return (
    <>
      ...
      <p key={item.team_bonus + civilStupid}>
        Командный бонус: {item.team_bonus}
      </p>

      {item.unique_unit && item.unique_unit[0] ? (
        item.unique_unit.map((linkUnit, i) => {
          return (
            <div
              key={"CivilsItem" + civilStupid + i}
              className={civilizations + "ItemDetail"}
            >
              <Link
                to={`${urlCiv}/unique_unit`}
                id="unique_unit"
                data-path={i}
                onClick={handleclick}
              >
                {i + 1}. Уникальный юнит {item.name}
              </Link>
            </div>
          );
        })
        ...
        <div
          key={"CivilsItemUnit" + civilStupid}
          className={`noItem forUnit${civilizations}`}
        >
          <p>У цивилизации нет уникальных юнитов</p>
        </div>
      )}

      {item.unique_tech && item.unique_tech[0] ? (
        item.unique_tech.map((linkTech, i) => {
          return (
            <div
              key={"CivilsItem2" + civilStupid + i}
              className={civilizations + "ItemDetail"}
            >
              <Link
                to={`${urlCiv}/unique_tech`}
                id="unique_tech"
                onClick={handleclick}
                data-path={i}
              >
                {i + 1}. Уникальная технология {item.name}
              </Link>
            </div>
          );
        })
        ...
        <div
          key={"CivilsItemNoTech" + civilStupid}
          className={`noItem forTech${civilizations}`}
        >
  ...
  return (
    <>
      <ul key={"Skils" + civDetailinfo} id="Skils">
        ...
        {$.attack_bonus ? (
          <ul key={"AttackBonus" + civDetailinfo} className="AttackBonus">
            <li>Бонус к атаке:</li>
            {$.attack_bonus &&
              $.attack_bonus.map((item, i) => {
                return <li key={item + civDetailinfo}>{item}</li>;
              })}
          </ul>
        ) : null}
      </ul>
  ...
  return (
    <>
      <ul key={"Skils" + civDetailinfo} id="Skils">
        ...
        {$.cost.Food || $.cost.Gold ? (
          <li>
            Стоимость постройки: {`${$.cost.Food} еды`} ,{" "}
            {`${$.cost.Gold} золота`}
          </li>
        ) : null}

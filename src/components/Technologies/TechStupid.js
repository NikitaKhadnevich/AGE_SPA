...
const TechInfoStupid = (props) => {
  const {
    children: [item, urlCiv, handleclick, technologies],
  } = props;
  ...
  return (
    <>
      ...
      <p>
        Стоимость Юнита: {item.cost.Wood} древесины, {item.cost.Gold} золота
      </p>
      {item.develops_in.length > 2 ? (
        <div
          key={"TechItem" + techStupid}
          className={technologies + "ItemDetail"}
        >
          <Link to={`${urlCiv}/tech`} id={item.id} onClick={handleclick}>
            Мануфакторум Юнита {item.name}
          </Link>
        </div>
      ) : (
        <div
          key={"TechItemNoTech" + techStupid}
          className={technologies + "NoItem"}
        >
        ...
        <>
          ...
          <li>
            <ul>
              ...
              Необходимые ресурсы:
              {item.cost.Wood ? (
                <li>{item.cost.Wood} древесины </li>
              ) : (
                <li>0 древесины</li>
              )}
              {item.cost.Gold ? (
                <li>{item.cost.Gold} золота </li>
              ) : (
                <li>0 золота</li>
              )}
            </ul>
          ...
          <ul key={"skils" + techDetailInfo} id={"skils" + techDetailInfo}>
            ...
            <li>
              <ul>
                {" "}
                Необходимые ресурсы:
                {dataDetail.cost.Wood ? (
                  <li>{dataDetail.cost.Wood} древесины</li>
                ) : (
                  <li> - древесины</li>
                )}
                {dataDetail.cost.Gold ? (
                  <li>{dataDetail.cost.Gold} золота</li>
                ) : (
                  <li> - золота</li>
                )}
              </ul>

...
export const UnitsStupid = (props) => {
  const {
    children: [item, urlCiv, handleclick, units],
  } = props;
  ...
  return (
    <>
      ...
      {item.reload_time ? (
        <p key={item.reload_time + unitSkil}>
          Время перезарядки: {item.reload_time} сек.
        </p>
      ) : null}
      <p key={item.movement_rate + unitSkil + "movement"}>
        Скорость передвижения: {item.movement_rate}
      </p>
      <p key={item.armor + unitSkil + "armor"}>Броня: {item.armor}</p>
      <p key={item.hit_points + unitSkil + "hit"}>
        Запас здоровья: {item.hit_points}
      </p>
      {item.range ? (
        <p key={item.range + unitSkil + "range"}>
          Дистанция атаки: {item.range}
        </p>
      ) : null}
      {item.build_time ? (
        <p key={item.build_time + unitSkil + "build_time"}>
          Время постройки: {item.build_time}
        </p>
      ) : null}
      <p key={item.age + unitSkil}>Тип: {item.age}</p>
      <p key={item.description + unitSkil}>
        Отличительная особенность: {item.description}
      </p>
      {item.cost.Wood ? (
        <p key={item.cost.Wood + unitSkil + "Wood"}>
          Стоимость в древесине: {item.cost.Wood} единиц
        </p>
      ) : null}
      {item.cost.Food ? (
        <p key={item.cost.Food + unitSkil + "Food"}>
          Затраты на создание в еде: {item.cost.Food} единиц
        </p>
      ) : null}
      {item.cost.Gold ? (
        <p key={item.cost.Gold + unitSkil + "Gold"}>
          Стоимость в золоте: {item.cost.Gold} монет
        </p>
      ) : null}
      {item.cost.info ? (
        <p key={item.cost.Gold + unitSkil}>Стоимость: {item.cost.info} </p>
      ) : null}
      {item.created_in.includes("https" || "http") ? (
        <div
          key={item.name + unitSkil + "building"}
          className={units + "ItemDetail"}
        >
          <Link to={`${urlCiv}/building`} id={item.id} onClick={handleclick}>
            Мануфакторум Юнита {item.name}
          </Link>
        </div>
      ) : (
        <div
          key={"UnitsItemNoTech" + unitsStupid}
          className={`noItem for${units}`}
        >
